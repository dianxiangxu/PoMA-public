...
//Add one operation
public class MutatorAOAR extends ProhibitionMutation {
	public MutatorAOAR(String testMethod, Graph graph, Prohibitions prohibitions) throws GraphDoesNotMatchTestSuitException, PMException {
	...
	public void main(Graph graph, Prohibitions prohibitions, String testMethod) throws PMException, IOException {
		...
		for (Prohibition p : prohibitionList) {
			...
			for (String newOperation : operationSet) {
				...
				if (before == after) {
					System.out.println("Mutant is not killed! Prohibition name:" + name +  " Added operation is " + newOperation);
				}
